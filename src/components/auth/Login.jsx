...
export default function Login({ onSwitch }) {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    // Simulate login success: close modal or show success (call onSwitch if needed)
    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-pink-200 via-purple-100 to-blue-200">
      <div className="backdrop-blur-xl bg-white/60 border border-pink-100 rounded-3xl shadow-2xl p-10 w-full max-w-md flex flex-col items-center animate-fade-in">
        ...
        <form className="w-full flex flex-col gap-6" onSubmit={handleSubmit}>
          ...
          <button type="submit" className="w-full py-3 rounded-lg bg-purple-600 text-white font-semibold hover:bg-purple-700 transition">Sign In</button>
        </form>
        <div className="mt-5 text-gray-500 text-sm">Don&apos;t have an account? <button className="text-purple-600 font-semibold hover:underline" onClick={onSwitch}>Sign Up</button></div>
      </div>
    </div>
  );
} 